...
def get_constraints(adata, celltype_priors, knn, celltypes):
    ...
    prior_genes = {}
    for celltype in set(celltypes):
        prior_genes[celltype] = np.array([columns.index(gene) for gene in adata.var_names])
    ...
    for cellrand in range(num_cells):
        ...
        gene_indices_to_take_from_data = prior_genes[celltypes[cellrand]]
        prior_pred = prior.dot(data.values[cellrand, gene_indices_to_take_from_data])
