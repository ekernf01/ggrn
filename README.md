...
GGRN is a set of Python modules that together allow users to flexibly combine traits of different methods for gene regulatory network inference. A full exposition is provided in `GGRN.md`, which is the raw source for the appendix from our  preprint (not up at time of writing). This is motivated by our [benchmarking project](https://ekernf01/perturbation_benchmarking).
