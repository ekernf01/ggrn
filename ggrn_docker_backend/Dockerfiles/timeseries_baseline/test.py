...
grn = ggrn.GRN(train)
grn.fit(
    method = "docker____ekernf01/ggrn_docker_backend_timeseries_baseline"
)
ad_out = grn.predict(predictions_metadata = test.obs.loc[np.random.choice(test.obs_names, 50), ['timepoint', 'cell_type', 'perturbation', "perturbation_type", "expression_level_after_perturbation", "is_control"]], prediction_timescale = 0)
ad_out = grn.predict(predictions_metadata = test.obs.loc[np.random.choice(test.obs_names, 50), ['timepoint', 'cell_type', 'perturbation', "perturbation_type", "expression_level_after_perturbation", "is_control"]], prediction_timescale = 1)
ad_out = grn.predict(predictions_metadata = test.obs.loc[np.random.choice(test.obs_names, 50), ['timepoint', 'cell_type', 'perturbation', "perturbation_type", "expression_level_after_perturbation", "is_control"]], prediction_timescale = 2)
