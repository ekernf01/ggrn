    ...
    def test_match_timeseries(self):
        ...
        for is_int in [True, False]:
            for mm in ["steady_state", "user",  "closest", "user", "random", "user", "optimal_transport", "user"]:
                ...
                has_control = grn.train.obs["matched_control"].notnull().astype(bool)
                # Everything that has a match should have a match at the same or an earlier timepoint.
                if not is_int:
                    matches = grn.train.obs.loc[:,"matched_control"][has_control]
                    match_timepoints = grn.train.obs.loc[matches, "timepoint"]
                    self.assertTrue(
                        all(match_timepoints.values <= grn.train.obs.loc[:, "timepoint"][has_control])
                    )
                if is_int: 
                    self.assertTrue(
                        all(grn.train.obs.loc[:,"matched_control"][has_control].isin(range(len(grn.train.obs_names))))
                    )
                else:
                    self.assertTrue(
                        all(grn.train.obs.loc[:,"matched_control"][has_control].isin(grn.train.obs_names))
                    )
